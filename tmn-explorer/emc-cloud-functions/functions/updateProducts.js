Moralis.Cloud.define("updateProducts", async(request) => {
  const expectedContentType = "Product";
    ...
    const obj = newResults[i];
      ...
      .then(function(httpResponse) {
        ...
        if (tokenType.toLowerCase() != expectedContentType.toLowerCase()) {
          ...
          const existsQuery = new Moralis.Query("Product");
          existsQuery.equalTo("owner", obj.get('address').toLowerCase())
          existsQuery.equalTo("productid", parseInt(data.productId))
          ...
          let doInsert = true;
            existsQuery.find({ useMasterKey: true }).then(function(results) {
              ...
              if (doInsert) {
                const nftPro = new Product();
                nftPro.save({
                  productid: parseInt(data.productId),
                  companyid: parseInt(data.companyId),
                  name: data.productName,
                  generics: data.productGenerics,
                  marketingauthnumb: data.productMAN,
                  dateOfauthorisation: data.productDateOfAuth,
                  subjecttoadditionalmon: data.productMonitoring,
                  authorisedincountries: data.productCountries,
